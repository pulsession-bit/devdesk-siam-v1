...
import { ShieldCheck, Globe, Zap, Award, Check, Calendar, Phone, Mail, Video, MessageCircle, ChevronDown, Clock, ArrowRight, User, Loader2, CheckCircle2, Hash } from 'lucide-react';
import { ThemeMode, AppStep } from '../types';
...
const LandingGeneral: React.FC<LandingGeneralProps> = ({ onLoginRequest, onStartConcierge, onNavigateToSecurity, onNavigateToDTV, onSearch }) => {
    ...
    const getDynamicWhatsAppUrl = (refId?: string) => {
        ...
        const dossierRef = refId ? `[DOSSIER: ${refId.toUpperCase()}]` : "[NOUVEAU DOSSIER]";
        ...
        <div className="min-h-screen bg-white font-sans overflow-y-auto scroll-container flex flex-col">
            <div className="relative bg-[#051229] min-h-[90vh] flex flex-col">
                <div className="absolute inset-0 z-0">
                    <img src="https://images.unsplash.com/photo-1552465011-b4e21bf6e79a?q=80&w=2600&auto=format&fit=crop" className="w-full h-full object-cover opacity-60" alt={t('alt.thailand_boats')} />
                    <div className="absolute inset-0 bg-gradient-to-b from-[#051229]/90 via-[#051229]/40 to-[#051229]" />
                </div>
            ...
            <div className="py-20 px-6 bg-white relative z-20 -mt-10 rounded-t-[3rem]">
                <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-12">
                    <div className="flex flex-col items-center text-center space-y-4 group"><div className="w-16 h-16 bg-[#F8FAFC] rounded-full flex items-center justify-center text-[#051229] mb-2 group-hover:bg-[#051229] group-hover:text-white transition-colors duration-300"><Globe size={28} strokeWidth={1.5} /></div><h3 className="font-black text-[#051229] text-lg uppercase tracking-wide">{t('welcome.feature_freedom')}</h3><p className="text-sm text-slate-500 max-w-[250px] leading-relaxed">{t('welcome.feature_freedom_desc')}</p></div>
                    <div className="flex flex-col items-center text-center space-y-4 group"><div className="w-16 h-16 bg-[#F8FAFC] rounded-full flex items-center justify-center text-[#051229] mb-2 group-hover:bg-[#051229] group-hover:text-white transition-colors duration-300"><Award size={28} strokeWidth={1.5} /></div><h3 className="font-black text-[#051229] text-lg uppercase tracking-wide">{t('welcome.feature_expert')}</h3><p className="text-sm text-slate-500 max-w-[250px] leading-relaxed">{t('welcome.feature_expert_desc')}</p></div>
                    <div className="flex flex-col items-center text-center space-y-4 group"><div className="w-16 h-16 bg-[#F8FAFC] rounded-full flex items-center justify-center text-[#051229] mb-2 group-hover:bg-[#051229] group-hover:text-white transition-colors duration-300"><Zap size={28} strokeWidth={1.5} /></div><h3 className="font-black text-[#051229] text-lg uppercase tracking-wide">{t('welcome.feature_speed')}</h3><p className="text-sm text-slate-500 max-w-[250px] leading-relaxed">{t('welcome.feature_speed_desc')}</p></div>
                </div>
            </div>
            <div ref={formRef} className="bg-[#F8FAFC] py-20 px-6 scroll-mt-20">
                <div className="max-w-6xl mx-auto bg-white rounded-[3rem] shadow-xl overflow-hidden flex flex-col lg:flex-row border border-slate-100">
                    <div className="lg:w-2/5 bg-[#051229] p-12 text-white flex flex-col justify-center relative overflow-hidden">
                        ...
                        <div className="relative z-10"><h2 className="text-3xl md:text-4xl font-black leading-tight mb-2">{t('landing.hero_title')} <br /> <span className="text-[#FF9F1C]">{t('landing.hero_highlight')}</span></h2><p className="text-slate-400 text-sm mb-10 leading-relaxed">{t('landing.hero_desc')}</p><ul className="space-y-4">{[t('landing.check_contract'), t('landing.check_embassy'), t('landing.check_guarantee')].map((item, i) => (<li key={i} className="flex items-center gap-3 text-sm font-bold"><div className="p-1 rounded-full bg-[#FF9F1C] text-[#051229]"><Check size={10} strokeWidth={4} /></div>{item}</li>))}</ul></div>
                    </div>
                    <div className="lg:w-3/5 p-8 md:p-12 bg-white flex flex-col justify-center">
                        {isSuccess ? (
                            <div className="text-center animate-in zoom-in duration-500">
                                ...
                                <h3 className="text-3xl font-black text-[#051229] mb-4">{t('landing.success_title')}</h3>
                                <p className="text-lg font-bold text-emerald-600 mb-6">{t('landing.success_subtitle')}</p>
                                {appointmentRef && (
                                ...
                                <p className="text-slate-500 max-w-md mx-auto mb-8 leading-relaxed">{t('landing.success_desc')}</p>
                                <button onClick={() => { setIsSuccess(false); setAppointmentRef(null); setFormData({ name: '', email: '', date: '', time: '', contactDetail: '' }); }} className="block w-full max-w-xs mx-auto py-3 bg-[#051229] text-white rounded-xl font-bold text-sm uppercase tracking-widest hover:bg-[#0F264A] transition-all">{t('landing.btn_home')}</button>
                            </div>
                        ) : (
                            <div className="max-w-md mx-auto w-full pb-12">
                                ...
                                <div className="space-y-5">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div><label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 block flex items-center gap-2"><User size={12} /> {t('landing.label_name')}</label><input type="text" placeholder="John Doe" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full bg-[#F8FAFC] border border-slate-200 rounded-xl px-4 py-3 text-sm font-medium text-[#051229] outline-none focus:border-[#051229] transition-colors" /></div>
                                        <div><label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 block flex items-center gap-2"><Mail size={12} /> {t('landing.label_email')}</label><input type="email" placeholder="john@email.com" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} className="w-full bg-[#F8FAFC] border border-slate-200 rounded-xl px-4 py-3 text-sm font-medium text-[#051229] outline-none focus:border-[#051229] transition-colors" /></div>
                                    </div>
                                    <div>
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div><label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 block flex items-center gap-2"><Clock size={12} /> {t('landing.label_time1')}</label><select value={formData.time} onChange={(e) => setFormData({ ...formData, time: e.target.value })} className="w-full bg-[#F8FAFC] border border-slate-200 rounded-xl px-4 py-3 text-sm font-medium text-[#051229] outline-none focus:border-[#051229] transition-colors appearance-none"><option value="">--:--</option><option value="10:00">10:00</option><option value="14:00">14:00</option></select></div>
                                        <div><label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 block flex items-center gap-2"><Clock size={12} /> {t('landing.label_time2')}</label><select className="w-full bg-[#F8FAFC] border border-slate-200 rounded-xl px-4 py-3 text-sm font-medium text-[#051229] outline-none focus:border-[#051229] transition-colors appearance-none"><option>--:--</option><option>11:00</option><option>15:00</option></select></div>
                                    </div>
                                    <div>
                                        ...
                                        <div className="grid grid-cols-4 gap-2">
                                            <button onClick={() => setContactMethod('whatsapp')} className={`flex flex-col items-center justify-center p-3 rounded-xl border transition-all ${contactMethod === 'whatsapp' ? 'border-[#FF9F1C] bg-[#FF9F1C]/10 text-[#051229]' : 'border-slate-100 text-slate-400 hover:bg-slate-50'}`}><MessageCircle size={18} className="mb-1" /><span className="text-[9px] font-bold">{t('landing.contact_methods.whatsapp')}</span></button>
                                            <button onClick={() => setContactMethod('phone')} className={`flex flex-col items-center justify-center p-3 rounded-xl border transition-all ${contactMethod === 'phone' ? 'border-[#FF9F1C] bg-[#FF9F1C]/10 text-[#051229]' : 'border-slate-100 text-slate-400 hover:bg-slate-50'}`}><Phone size={18} className="mb-1" /><span className="text-[9px] font-bold">{t('landing.contact_methods.phone')}</span></button>
                                            <button onClick={() => setContactMethod('email')} className={`flex flex-col items-center justify-center p-3 rounded-xl border transition-all ${contactMethod === 'email' ? 'border-[#FF9F1C] bg-[#FF9F1C]/10 text-[#051229]' : 'border-slate-100 text-slate-400 hover:bg-slate-50'}`}><Mail size={18} className="mb-1" /><span className="text-[9px] font-bold">{t('landing.contact_methods.email')}</span></button>
                                            <button onClick={() => setContactMethod('zoom')} className={`flex flex-col items-center justify-center p-3 rounded-xl border transition-all ${contactMethod === 'zoom' ? 'border-[#FF9F1C] bg-[#FF9F1C]/10 text-[#051229]' : 'border-slate-100 text-slate-400 hover:bg-slate-50'}`}><Video size={18} className="mb-1" /><span className="text-[9px] font-bold">{t('landing.contact_methods.zoom')}</span></button>
                                        </div>
                                    </div>
                                    <input type="text" placeholder={t('landing.placeholder_contact')} value={formData.contactDetail} onChange={(e) => setFormData({ ...formData, contactDetail: e.target.value })} className="w-full bg-[#F8FAFC] border border-slate-200 rounded-xl px-4 py-4 text-sm font-medium text-[#051229] outline-none focus:border-[#051229] transition-colors" />
                                    <button onClick={handleSubmit} disabled={isSubmitting} className="w-full bg-[#051229] hover:bg-[#0F264A] text-white py-5 rounded-2xl font-black text-xs uppercase tracking-[0.2em] shadow-xl transition-all flex items-center justify-center gap-3 mt-4 disabled:opacity-80 disabled:cursor-not-allowed active:scale-[0.98] border-b-4 border-[#FF9F1C]">{isSubmitting ? (<><Loader2 size={16} className="animate-spin" />{t('landing.btn_processing')}</>) : t('landing.btn_submit')}</button>
